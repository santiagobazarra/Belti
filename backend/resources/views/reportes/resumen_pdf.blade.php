...
<head>
    ...
    <title>Reporte Resumen General - Belti</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        /* Keep section header and table together */
        .section-container {
            page-break-inside: avoid;
            margin-bottom: 20px;
        }
        ...
        @media print {
            ...
            /* Only break sections if absolutely necessary */
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-info">
            <div class="report-title">Reporte Resumen General</div>
            <div class="report-period">Período: {{ $desde }} - {{ $hasta }}</div>
        </div>
    </div>

    <div class="summary-section">
        <div class="summary-title">Resumen General del Período</div>
        <table class="summary-table">
            ...
            <tbody>
                <tr>
                    <td>
                        <span class="summary-number">{{ $jornadas->count() }}</span>
                    ...
                    <td>
                        <span class="summary-number">{{ $incidencias->count() }}</span>
                        <span class="summary-label">incidencias</span>
                    </td>
                    <td>
                        <span class="summary-number">{{ number_format($jornadas->sum('horas_trabajadas'), 1) }}</span>
    ...
    <div class="section-container">
        <div class="section-header">Jornadas Laborales</div>
        ...
        <tbody>
            @forelse($jornadas->take(10) as $jornada)
                <tr>
                    <td class="employee-name">{{ $jornada->usuario->nombre ?? 'N/A' }}</td>
                    <td>{{ $jornada->usuario->departamento->nombre ?? 'N/A' }}</td>
                    <td class="date-cell">{{ \Carbon\Carbon::parse($jornada->fecha)->format('d/m/Y') }}</td>
                    <td class="time-cell">{{ $jornada->hora_entrada ? \Carbon\Carbon::parse($jornada->hora_entrada)->format('H:i') : '-' }}</td>
                    <td class="time-cell">{{ $jornada->hora_salida ? \Carbon\Carbon::parse($jornada->hora_salida)->format('H:i') : '-' }}</td>
                    <td class="time-cell">{{ $jornada->horas_trabajadas ? number_format($jornada->horas_trabajadas, 2) . 'h' : '-' }}</td>
                    <td class="status-cell">
                        <span class="status-badge status-{{ $jornada->estado === 'completa' ? 'completa' : 'incompleta' }}">
                            {{ ucfirst($jornada->estado) }}
                        </span>
                    </td>
                    <td>
                        @if($jornada->estado === 'incompleta')
                            Fichaje incompleto
                        @elseif($jornada->horas_trabajadas && $jornada->horas_trabajadas > 8)
                            Horas extra
                        @else
                            -
                        @endif
            ...
            @empty
                <tr>
                    <td colspan="8" class="no-data">
                        No se encontraron jornadas para el período seleccionado
                    </td>
                </tr>
            @endforelse
        </tbody>
    ...
    <div class="section-container">
        <div class="section-header">Solicitudes</div>
        <table class="data-table">
        <thead>
            <tr>
        ...
        <tbody>
            @forelse($solicitudes->take(10) as $solicitud)
                <tr>
                    ...
                    <td>
                        @if($solicitud->motivo_rechazo)
                            {{ Str::limit($solicitud->motivo_rechazo, 200) }}
    ...
    <div class="section-container">
        <div class="section-header">Incidencias</div>
        <table class="data-table">
        <thead>
            <tr>
                <th style="width: 25%;">Empleado</th>
                <th style="width: 20%;">Tipo</th>
                <th style="width: 12%;">Fecha Reporte</th>
                <th style="width: 12%;">Fecha Resolución</th>
                <th style="width: 10%;">Estado</th>
                <th style="width: 21%;">Descripción</th>
            </tr>
        </thead>
        <tbody>
            @forelse($incidencias->take(10) as $incidencia)
                <tr>
                    <td class="employee-name">{{ $incidencia->usuario->nombre ?? 'N/A' }}</td>
                    <td class="type-cell">{{ ucfirst($incidencia->tipo) }}</td>
                    <td class="date-cell">{{ \Carbon\Carbon::parse($incidencia->fecha_reporte)->format('d/m/Y') }}</td>
                    <td class="date-cell">{{ $incidencia->fecha_resolucion ? \Carbon\Carbon::parse($incidencia->fecha_resolucion)->format('d/m/Y') : '-' }}</td>
                    <td class="status-cell">
                        <span class="status-badge status-{{ str_replace('_', '-', $incidencia->estado) }}">
                            {{ ucfirst(str_replace('_', ' ', $incidencia->estado)) }}
                        </span>
                    </td>
                    <td>{{ Str::limit($incidencia->descripcion, 200) }}</td>
                </tr>
            @empty
                <tr>
    ...
    <div class="footer">
        ...
        <p><em>Nota: Se muestran los primeros 10 registros de cada categoría. Para ver todos los registros, genere reportes específicos.</em></p>
    </div>
</body>
</html>
