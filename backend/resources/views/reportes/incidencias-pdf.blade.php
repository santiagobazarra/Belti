...
<head>
    ...
    <title>Reporte de Incidencias - Belti</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <table class="summary-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <span class="summary-number" style="color: #e65100;">{{ $incidencias->where('estado', 'pendiente')->count() }}</span>
                        <span class="summary-label">pendientes</span>
                    </td>
                    <td>
                        <span class="summary-number" style="color: #1565c0;">{{ $incidencias->where('estado', 'en_proceso')->count() }}</span>
                        <span class="summary-label">en proceso</span>
                    </td>
                    <td>
                        <span class="summary-number" style="color: #2e7d32;">{{ $incidencias->where('estado', 'resuelta')->count() }}</span>
                        <span class="summary-label">resueltas</span>
                    </td>
                    <td>
                        <span class="summary-number" style="color: #c62828;">{{ $incidencias->where('estado', 'cancelada')->count() }}</span>
        ...
        <tbody>
            @forelse($incidencias as $incidencia)
                <tr>
                    ...
                    <td class="priority-cell priority-{{ $incidencia->tipo }}">
                        {{ $incidencia->hora_inicio ? \Carbon\Carbon::parse($incidencia->hora_inicio)->format('H:i') : '-' }}
                        @if($incidencia->hora_fin)
                            - {{ \Carbon\Carbon::parse($incidencia->hora_fin)->format('H:i') }}
                        @endif
                    </td>
                    <td class="date-cell">{{ \Carbon\Carbon::parse($incidencia->fecha)->format('d/m/Y') }}</td>
                    <td class="date-cell">{{ $incidencia->fecha_revision ? \Carbon\Carbon::parse($incidencia->fecha_revision)->format('d/m/Y') : '-' }}</td>
