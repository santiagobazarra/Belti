...
<head>
    ...
    <title>Reporte de Jornadas - Belti</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-info">
            <div class="report-title">Reporte de Jornadas Laborales</div>
        ...
        <table class="summary-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <span class="summary-number" style="color: #2e7d32;">{{ $jornadas->where('estado', 'completa')->count() }}</span>
                        <span class="summary-label">completas</span>
                    </td>
                    <td>
                        <span class="summary-number">{{ number_format($jornadas->sum('total_horas'), 1) }}</span>
                        <span class="summary-label">horas</span>
                    </td>
                    <td>
                        <span class="summary-number" style="color: #e65100;">{{ number_format($jornadas->sum('horas_extra'), 1) }}</span>
                        <span class="summary-label">horas</span>
                    </td>
                    <td>
                        <span class="summary-number" style="color: #666666;">
                            @php
                                $totalPausas = 0;
                                foreach($jornadas as $jornada) {
                                    foreach($jornada->pausas as $pausa) {
                                        if($pausa->hora_inicio && $pausa->hora_fin) {
                                            ...
                                            $totalPausas += $inicio->diffInMinutes($fin);
                                        }
                                    }
                                }
                                echo number_format($totalPausas / 60, 1);
                            @endphp
                        </span>
                        <span class="summary-label">horas</span>
                    </td>
        ...
        <tbody>
            @forelse($jornadas as $jornada)
                <tr>
                    ...
                    <td>{{ $jornada->usuario->departamento->nombre ?? 'Sin departamento' }}</td>
                    ...
                    <td class="time-cell">{{ $jornada->total_horas ? number_format($jornada->total_horas, 2) : '-' }}</td>
                    <td class="time-cell">
                        @php
                            $totalPausas = 0;
                            $pausasCount = 0;
                        ...
                        @endphp
                        @if($totalPausas > 0)
                            {{ number_format($totalPausas / 60, 1) }}h ({{ $pausasCount }})
                        @else
                            -
                        @endif
                    </td>
                    <td class="status-cell">
                        <span class="status-badge status-{{ strtolower($jornada->estado) }}">
                    ...
                    <td>
                        @if($jornada->estado === 'abierta')
                            Jornada en curso
                        @elseif($jornada->horas_extra && $jornada->horas_extra > 0)
                            Horas extra: {{ number_format($jornada->horas_extra, 2) }}h
                        @elseif($totalPausas > 0)
                            {{ $pausasCount }} pausa(s) registrada(s)
                        @else
                            Normal
                        @endif
                    </td>
