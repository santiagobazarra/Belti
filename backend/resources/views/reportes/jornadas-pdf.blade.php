...
<body>
    ...
    <div class="summary-section">
        ...
        <table class="summary-table">
            ...
            <tr>
                <td><span class="summary-number">{{ $jornadas->count() }}</span></td>
                <td><span class="summary-number">{{ $jornadas->where('estado', 'completa')->count() }}</span></td>
                <td><span class="summary-number">{{ $jornadas->where('estado', 'incompleta')->count() }}</span></td>
                <td><span class="summary-number">{{ number_format($jornadas->sum('horas_trabajadas'), 1) }}h</span></td>
            </tr>
        </table>
    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($jornadas as $jornada)
                <tr>
                    <td>{{ $jornada->usuario->nombre ?? 'N/A' }}</td>
                    <td>{{ $jornada->usuario->departamento->nombre ?? 'N/A' }}</td>
                    <td>{{ \Carbon\Carbon::parse($jornada->fecha)->format('d/m/Y') }}</td>
                    <td>{{ $jornada->hora_entrada ? \Carbon\Carbon::parse($jornada->hora_entrada)->format('H:i') : '-' }}</td>
                    <td>{{ $jornada->hora_salida ? \Carbon\Carbon::parse($jornada->hora_salida)->format('H:i') : '-' }}</td>
                    <td>{{ $jornada->horas_trabajadas ? number_format($jornada->horas_trabajadas, 2) . 'h' : '-' }}</td>
                    <td>
                        <span class="status-badge {{ $jornada->estado === 'completa' ? 'status-completa' : 'status-incompleta' }}">
